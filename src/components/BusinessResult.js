...
export default function BusinessResult({
  ...
  const svg = (
    <svg width="16" height="16" className="icon_svg">
      <path
        fill="#2eb187"
        d="M6.308 11.763a.748.748 0 01-.53-.22l-2.641-2.64a.75.75 0 011.06-1.061l2.11 2.11 5.496-5.495a.75.75 0 111.06 1.06l-6.025 6.026a.748.748 0 01-.53.22z"
      ></path>
    </svg>
  );

  const transactionsJsx = transactionsArray.map((t, index) => (
  ...
  return (
    <div
      ...
      className={`${styles.listing} softBottomBorder centered`}
    >
      <img className={styles.img} src={image_url} alt="Restaurant or food" />
      <div className={styles.content}>
        <p className={styles.link} to={`/businesses/${id}`}>
          {" "}
          {name}
        </p>
        <div>
        ...
        <div className={styles.commentBox}>
          ...
          <div className={styles.comment}>
            "{comment}" <Link to={`/business/${id}`}>more</Link>
          </div>
        </div>
        <div>{address}</div>
        <div className={styles.transactionBox}>{transactionsJsx}</div>
      </div>
    </div>
  );
}
